...
function addPost() {
    ...
    if (postImage) {
        ...
        reader.onload = function (e) {
            ...
            renderPost(newPost);
            filterPosts(); // Llama a la función de filtrado después de agregar la tarjeta
        };
...
function filterPosts() {
  const selectedOption = document.getElementById('typePostSelect').value;

  // Obtener todas las tarjetas de publicación
  const cards = document.querySelectorAll('.card');

  cards.forEach(card => {
      const postTypeText = card.querySelector('.viajePublicacion .badge').textContent.trim(); // Obtener el texto del tipo de publicación
      if (selectedOption === 'all' || postTypeText === selectedOption) {
...
document.getElementById('typePostSelect').addEventListener('change', filterPosts);
